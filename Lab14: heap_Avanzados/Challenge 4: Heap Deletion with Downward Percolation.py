...
class MinHeap:
    ...
    def delete_min(self):
        if not self.heap:
            return None  # 1.4.1: Empty heap

        if len(self.heap) == 1:
            return self.heap.pop()  # 1.4.2: Single element

        min_val = self.heap[0]
        self.heap[0] = self.heap.pop()  # Reemplazar raíz con el último
        self._heapify_down(0)  # Restaurar propiedad min-heap
        return min_val
