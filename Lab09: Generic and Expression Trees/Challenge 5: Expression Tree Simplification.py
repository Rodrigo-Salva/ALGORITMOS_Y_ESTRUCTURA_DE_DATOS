        ...
        def simplify_node(node):
            ...
            if node.left and node.right:
                if is_constant(node.left.value) and is_constant(node.right.value):
                    result = evaluate(node.value, node.left.value, node.right.value)
